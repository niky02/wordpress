...
if(current_user_can('administrator')){
	...
	<div class="wrap">
		...
		<div>
			<h3>批量操作</h3>
			<form method="post">给所有VIP都延长（减少）<input type="text" name="adddate" placeholder="整数天数">天的VIP权限 <input type="submit" value="确定操作"><input type="hidden" name="action" value="1"> （输入负数表示减少天数）</form><br>
			<h3>单用户查询</h3>
			<form method="post">搜索用户：<input type="text" name="username" placeholder="例如：admin"><input type="submit" value="查询"><input type="hidden" name="action" value="2"></form><br>
		</div>
			...
			<tbody>
				...
				if($list) {
					foreach($list as $value){
						...
						echo "<td>$typeName</td>\n";
						echo "<td><input type=text name=p_price_$value->ice_id id=p_price_$value->ice_id value=$value->endTime style=\"width:120px;\" /><input type=button id=editpricebtn_$value->ice_id onclick=editPrice($value->ice_id) value=修改 style=border:0px;color:#21759b;cursor:pointer></td>";
