                ...
                <?php if (_cao('is_custom_post_meta_opt', '0') && _cao('custom_post_meta_opt', '0')) {
                    ...
                    foreach ($custom_post_meta_opt as $filter) {
                        ...
                        if (!$opt_meta_category || in_array($currentterm_id, $opt_meta_category)) {
                            ...
                            $content = '<ul class="filter-tag"><span><i class="fa fa-folder-open-o"></i> ' . $filter['meta_name'] . '</span>';
                            ...
                            <div class="col-12 col-sm-6">
                                <?php if (_cao('is_filter_item_price', '1')) :
                                    ...
                                    foreach ($caotype_arr as $key => $item) {
                                        $_oncss = ($is_on == $key) ? 'on' : '';
                                        $content .= '<li><a href="' . preg_replace_link(add_query_arg("cao_type", $key)) . '" class="tab ' . $_oncss . '"><i></i><em>' . $item . '</em></a></li>';
                                    }
                                    $content .= "</ul>";
                                    echo $content;
                                endif; ?>
                            ...
                            <div class="col-12 col-sm-6">
                                ...
                                <?php if (_cao('is_filter_item_order', '1')) :
                                    ...
                                    foreach ($order_arr as $key => $item) {
                                        $_oncss = ($is_on == $key) ? 'on' : '';
                                        $content .= '<li class="rightss"><i class="fa fa-caret-down"></i> <a href="' . preg_replace_link(add_query_arg("order", $key)) . '" class="' . $_oncss . '">' . $item . '</a></li>';
                                    }
                                    $content .= "</div></ul>";
                                    echo $content;
                                endif; ?>
