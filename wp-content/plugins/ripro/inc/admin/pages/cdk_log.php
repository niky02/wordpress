...
$outcdk = (!empty($_GET['action']) && $_GET['action'] =='outcdk') ? true :false;
...
<?php
if ($outcdk) {
	$sql = "SELECT * FROM {$coupon_table_name} WHERE status=0 ORDER BY id DESC LIMIT 200";
	$result = $wpdb->get_results($sql);
	$content = '<div class="wrap"><h1 class="wp-heading-inline">导出最新200条未被使用的cdk</h1><textarea name="moderation_keys" rows="30" cols="50" id="moderation_keys" class="large-text code">';
	foreach ($result as $key => $txt) {
		$content .=$txt->code.PHP_EOL;  //\r\n 为换行
	}
...
<div class="wrap">
    ...
    <?php 
    	$add_url = add_query_arg(array('page' => $_GET['page'],'action' => 'add'), admin_url('admin.php'));
    	$outcdk = add_query_arg(array('page' => $_GET['page'],'action' => 'outcdk'), admin_url('admin.php'));
    ?>
    <a href="<?php echo $add_url ?>" class="page-title-action">添加卡密-CDK</a>
    <a href="<?php echo $outcdk ?>" class="page-title-action">导出卡密</a>
