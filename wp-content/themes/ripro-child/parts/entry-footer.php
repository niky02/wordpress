<div class="entry-footer">
    <ul class="post-meta-box">
        ...
        <?php endif; ?>
        <li class="vip-price"> <?php echo _get_post_cao_is_boosvip(); ?></li>
        <?php if (is_site_shop_open()) : ?>
            <?php if ((_get_post_shop_status() || _get_post_shop_hide() || _get_post_video_status() || true) && _cao('grid_is_price', true)) :
                $post_price = _get_post_price();
                $post_price = ($post_price) ? $post_price : '免费';
                ?>
                <?php if (!_cao('is_article_list_down')) { ?>
                <li class="meta-price">
                    <span><?php echo '<i class="' . _cao('site_money_icon') . '"></i> ' . $post_price; ?></span></li>
            <?php } else { ?>
                <li class="meta-price"><span><i class="fa fa-download"></i>下载</span></li>
            <?php } ?>
            <?php endif; ?>
        <?php endif; ?>
    </ul>
