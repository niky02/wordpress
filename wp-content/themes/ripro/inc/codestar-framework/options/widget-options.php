...
if (!function_exists('cao_widget_pay')) {
    function cao_widget_pay($args, $instance)
    {
        ...
        if ($cao_price==0) {
            ...
            echo '<b><span class="price"><span style=" color: #dadada; font-size: 15px; margin: 0;vertical-align:middle; ">原价：</span>' . $cao_price . '<sup>'. $site_money_ua.'</sup></span></b>';
        }
        ...
        if ($min_price < $cao_price || $cao_close_novip_pay) {
            ...
            if ($cao_close_novip_pay) {
                echo '<li><i class="fa fa-times-circle-o"></i> 普通用户暂无购买权限  <a class="label label-warning" href="/user?action=vip" class="pay-vip">升级'.$site_vip_name.'</a></li>';
            }else{
